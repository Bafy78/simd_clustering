...
def case_tag(alg: str, D: int, gmm_covariance_type: str | None = None) -> str:
    covariance_suffix = (
        f".{gmm_covariance_type}" if alg == "gmm_static" and gmm_covariance_type else ""
    )
    return f"{alg}{covariance_suffix}.{D}D"
...
def godbolt_source_path(
    ...
    return out_dir / f"godbolt.{case_tag(alg, D, gmm_covariance_type)}.cpp"
...
def emit_godbolt_source(
    *,
    ...
    output_path = godbolt_source_path(alg, D, out_dir, gmm_covariance_type)
    output_path.parent.mkdir(parents=True, exist_ok=True)

    assembly = spill_detector_assembly_path(alg, D, out_dir, gmm_covariance_type)
    ...
    header = [
        ...
        f"// Case: {alg}{covariance_label} {D}D",
    ...
    print(f"Wrote Godbolt source for {alg}{covariance_label} {D}D: {output_path}")
    return output_path
...
def compile_assembly(
    ...
    covariance_label = f" {gmm_covariance_type}" if gmm_covariance_type else ""
    print(f"Compiling assembly for {alg}{covariance_label} {D}D...")
    result = subprocess.run(cmd, cwd=REPO_ROOT, text=True, capture_output=True)

    if result.returncode != 0:
        print(f"\nAssembly compile failed for {alg}{covariance_label} {D}D")
...
def run_rg_scan(
    *,
    ...
    tag = case_tag(alg, D, gmm_covariance_type)
    output_path = out_dir / f"rg.{tag}.txt"
    cmd = [rg, "-nUP", pattern, str(assembly)]

    covariance_label = f" {gmm_covariance_type}" if gmm_covariance_type else ""
    print(f"Scanning assembly for {alg}{covariance_label} {D}D...")
    result = subprocess.run(cmd, cwd=REPO_ROOT, text=True, capture_output=True)
    ...
    if result.returncode not in (0, 1):
        print(f"\nrg failed for {alg}{covariance_label} {D}D")
        print(f"Command: {' '.join(cmd)}")
...
def write_summary(results: list[SpillScanResult], out_dir: Path) -> tuple[Path, Path]:
    ...
    lines = [
        ...
        "",
        f"{'alg':<{alg_width}}  {'gmm_covariance_type':<{cov_width}}  {'D':>{dim_width}}  "
    ...
    for result in sorted(
        results,
        key=lambda item: (item.alg, item.gmm_covariance_type or "", item.D),
    ):
        lines.append(
            ...
            for D in args.D_values:
                if args.skip_compile:
                    ...
                    if not assembly.exists():
                        ...
                        print(
                            f"Missing assembly file for {alg}{covariance_label} {D}D: {assembly}"
                        )
                        sys.exit(1)
                else:
                    assembly = compile_assembly(alg, D, out_dir, gmm_covariance_type)
