...
def case_tag(alg: str, dim: int, gmm_covariance_type: str | None = None) -> str:
    covariance_suffix = (
        f".{gmm_covariance_type}" if alg == "gmm_static" and gmm_covariance_type else ""
    )
    return f"{alg}{covariance_suffix}.{dim}D"
...
def godbolt_source_path(
    ...
    return out_dir / f"godbolt.{case_tag(alg, dim, gmm_covariance_type)}.cpp"
...
def compile_command_source(cmd: Iterable[str]) -> Path:
    sources = [Path(token) for token in cmd if Path(token).suffix in {".cc", ".cpp", ".cxx"}]
    if len(sources) != 1:
        raise RuntimeError(
...
def strip_include_delimiters(include_target: str) -> str | None:
    if len(include_target) >= 2 and include_target[0] == '"' and include_target[-1] == '"':
...
def resolve_local_include(
    *,
    ...
    for directory in search_dirs:
        ...
        if candidate.exists() and candidate.is_file() and is_repo_local_cpp_file(candidate):
            return candidate
...
def emit_godbolt_source(
    *,
    ...
    output_path = godbolt_source_path(alg, dim, out_dir, gmm_covariance_type)
    output_path.parent.mkdir(parents=True, exist_ok=True)

    assembly = spill_detector_assembly_path(alg, dim, out_dir, gmm_covariance_type)
    ...
    header = [
        ...
        f"// Case: {alg}{covariance_label} {dim}D",
    ...
    print(f"Wrote Godbolt source for {alg}{covariance_label} {dim}D: {output_path}")
    return output_path
...
def compile_assembly(
    ...
    covariance_label = f" {gmm_covariance_type}" if gmm_covariance_type else ""
    print(f"Compiling assembly for {alg}{covariance_label} {dim}D...")
    result = subprocess.run(cmd, cwd=REPO_ROOT, text=True, capture_output=True)

    if result.returncode != 0:
        print(f"\nAssembly compile failed for {alg}{covariance_label} {dim}D")
...
def run_rg_scan(
    *,
    ...
    tag = case_tag(alg, dim, gmm_covariance_type)
    output_path = out_dir / f"rg.{tag}.txt"
    cmd = [rg, "-nUP", pattern, str(assembly)]

    covariance_label = f" {gmm_covariance_type}" if gmm_covariance_type else ""
    print(f"Scanning assembly for {alg}{covariance_label} {dim}D...")
    result = subprocess.run(cmd, cwd=REPO_ROOT, text=True, capture_output=True)
    ...
    if result.returncode not in (0, 1):
        print(f"\nrg failed for {alg}{covariance_label} {dim}D")
        print(f"Command: {' '.join(cmd)}")
...
def write_summary(results: list[SpillScanResult], out_dir: Path) -> tuple[Path, Path]:
    ...
    if results:
        alg_width = max(len("alg"), *(len(result.alg) for result in results))
        cov_width = max(
            len("gmm_covariance_type"),
            *(len(result.gmm_covariance_type or "-") for result in results),
        )
        dim_width = max(len("dim"), *(len(str(result.dim)) for result in results))
        count_width = max(
    ...
    lines = [
        ...
        "",
        f"{'alg':<{alg_width}}  {'gmm_covariance_type':<{cov_width}}  {'dim':>{dim_width}}  "
        f"{'candidate_reload_pairs':>{count_width}}  rg_output",
        f"{'-' * alg_width}  {'-' * cov_width}  {'-' * dim_width}  {'-' * count_width}  ---------",
    ]

    for result in sorted(
        results,
        key=lambda item: (item.alg, item.gmm_covariance_type or "", item.dim),
    ):
        lines.append(
            f"{result.alg:<{alg_width}}  "
            f"{(result.gmm_covariance_type or '-'):<{cov_width}}  "
            f"{result.dim:>{dim_width}}  "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compile C++ benchmark cases to Intel-syntax assembly across dimensions, "
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        default=default_config().test_dimensions,
        help="Dimensions to compile and scan. Defaults to the benchmark config dimensions.",
    )
    parser.add_argument(
...
def scan_variants(alg: str, gmm_covariance_types: list[str]) -> list[str | None]:
    if alg == "gmm_static":
...
def main() -> None:
    ...
    for alg in args.alg:
        for gmm_covariance_type in scan_variants(alg, args.gmm_covariance_types):
            for dim in args.dims:
                if args.skip_compile:
                    ...
                    if not assembly.exists():
                        ...
                        print(
                            f"Missing assembly file for {alg}{covariance_label} {dim}D: {assembly}"
                        )
                        sys.exit(1)
                else:
                    assembly = compile_assembly(alg, dim, out_dir, gmm_covariance_type)
