...
int main(int argc, char* argv[]) {
    if (argc < 7) {
        std::cerr << "Usage: " << argv[0]
                << " <binary_file> <n_samples> <n_clusters> <nanobench_json_out> <bench_epochs> <min_epoch_seconds>\n";
        return 1;
    }
    ...
    std::size_t bench_epochs = std::stoull(argv[5]);
    double min_epoch_seconds = std::stod(argv[6]);
